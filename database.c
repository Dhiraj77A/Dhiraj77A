...
int authenticate_user(char *username, char *password, const char *role, User *user) {
    ...
    fclose(fp);
    printf("User not found or role mismatch.\n");
    return 0;
}
...
int create_user(char *username, char *password, char *name, int is_admin) {
    if (!validate_string(username, MAX_USERNAME) ||
        !validate_string(password, MAX_PASSWORD) ||
        !validate_string(name, MAX_NAME)) {
        printf("Invalid input: Username, password, and name must be non-empty, less than max length, and contain no spaces.\n");
        return 0;
    }
    ...
    strcpy(new_user.role, is_admin ? "Admin" : "Student");  // Simplified assignment
...
void load_session(Session *session) {
    ...
    if (fp) {
        fread(session, sizeof(Session), 1, fp);
        fclose(fp);
    }
}
